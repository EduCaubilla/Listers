...
struct CategoriesProductsView: View {
    ...
    var body: some View {
        VStack (alignment: .leading, spacing: 0) {
            ScrollViewReader { value in
                VStack{
                    if showSearchBar {
                        VStack(alignment: .leading, spacing: 0){
                            SearchBarCustomView(name: $name, showSearchBar: $showSearchBar, productNameList: vm.productNames, resultAction: {vm.scrollToFoundProduct(proxy: value, name: name)})
                        }
                        .padding(.bottom, 0)
                    }
                ...
                .sheet(isPresented: $vm.showingAddProductView) {
                    FormProductView(vm: vm, scrollViewProxy: value)
                        .padding(.top, 20)
                        .presentationDetents([.height(320)])
                        .presentationBackground(Color.background)
                }
                .sheet(isPresented: $vm.showingUpdateProductView) {
                    FormProductView(product: vm.selectedProduct, vm: vm, scrollViewProxy: value)
                        .padding(.top, 20)
                        .presentationDetents([.height(320)])
                        .presentationBackground(Color.background)
                }
        ...
        .sheet(isPresented: $vm.showingListToAddProductView) {
